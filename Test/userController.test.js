...
beforeEach(async () => {
	...
	dbCollection = await db.collection("users");
...
describe("POST users/signup", () => {
	...
	test("Sign up | No firstname | Should return 400", async () => {
	...
	test("Sign up | No lastname | Should return 400", async () => {
		user1.lastname = undefined
		const response = await request
			.post(`/users/signup`)
			.send(user2);
		expect(response.status).toBe(400)
